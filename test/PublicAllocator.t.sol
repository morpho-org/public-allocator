...
contract PublicAllocatorTest is IntegrationTest {
    ...
    function testReallocateCapZeroOutflowByDefault(uint flow) public {
    ...
    function testFeeAmountSuccess(uint requiredFee, uint givenFee) public {
        vm.prank(OWNER);
        publicAllocator.setFee(requiredFee);

        givenFee = bound(givenFee,requiredFee,type(uint).max);
    ...
    function testFeeAmountFail(uint requiredFee, uint givenFee) public {
